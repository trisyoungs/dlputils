!	** cn2 **
!	Calculate the coordination number over a subset of molecules (COM)
	...
	nargs = iargc()
	if (nargs.lt.6) then
	  write(0,"(A)") "Usage : cn2 <DLP HISTORYfile> <DLP OUTPUTfile> <framestodo> <species> <comatom or 0> <[-]selector> [extra data]"
	  stop
	end if
