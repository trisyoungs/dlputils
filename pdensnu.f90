...
!	image before binning
	...
	logical, allocatable :: axisdefined(:)		! Flags to indicate whether molecular axes have been defined
	integer :: baselen,nargs,nframesused,nmapframes
	character*80 :: hisfile,outfile,basename,resfile,temp,flagfile,altheaderfile
	character*8 :: discard
	logical :: altheader = .FALSE.
	integer :: success,calctype,bin,X,numadded,n1,n2,n3
	integer :: iatm, n, discardn, nframes,k, l, m, o, p, pairsp, pairmol
	real*8 :: px,py,pz, summ
	real*8 :: mindist, maxdist, dist, maxpairdist, mindistsq, maxdistsq, maxpairdistsq
	...
	if (nargs.LT.2) then
	  ...
	  write(*,"(a)") "        [-pair sp maxdist]      Only consider central molecules close to molecules of another species"
	  stop
	else
...
50	if (baselen.eq.-1) then
	...
	do
	  ...
	  select case (temp)
	    ...
	    case ("-header")
              ...
              altheader = .TRUE.
	    case ("-pair")
	      n = n + 1; call getarg(n,temp); read(temp,"(i4)") pairsp
	      n = n + 1; call getarg(n,temp); read(temp,"(f10.4)") maxpairdist
	      write(0,"(a,i2,a,f8.4)") "Pair separation in effect - max dist from species ",pairsp," = ",maxpairdist
	    case default
	...
	do sp1=1,nspecies
	  ...
	  if (axisdefined(sp1)) then
	    ...
	    if (s_natoms(sp1).gt.2) stop "Axes must be defined for all molecular species."
	  end if
	end do
...
102	success=readframe()
	...
	do m1=1,s_nmols(sp1)      ! Loop over all molecules of species 1...
	  ...
	  ! If the pair effect is in operation, only include this centre if a second molecule (from pairsp)
	  ! is within 'pairmaxdist' Angstroms
	  if (pairsp.gt.0) then
	    pairmol = 0
	    do m2=1,s_nmols(pairsp)
	      call pbc(axisox(pairsp,m2),axisoy(pairsp,m2),axisoz(pairsp,m2),axisox(sp1,m1),axisoy(sp1,m1),axisoz(sp1,m1),tx,ty,tz)
	      tx = tx - axisox(sp1,m1)
	      ty = ty - axisoy(sp1,m1)
	      tz = tz - axisoz(sp1,m1)
	      dist = tx*tx + ty*ty + tz*tz
	      if (dist.lt.maxpairdistsq) then
		pairmol = m2
		exit
	      end if
	    end do
	    if (pairmol.eq.0) cycle
	  end if
	  ...
	  do sp2=1,nspecies
	    do m2=1,s_nmols(sp2)
	      ...
	      dist = tx*tx + ty*ty + tz*tz
	      if (dist.lt.mindistsq) cycle
	      if (dist.gt.maxdistsq) cycle
...
120	write(0,*) "Finished."
	...
	do sp1=1,nspecies
	  ...
	  spexp(sp1) = n3dcentres * s_nmols(sp1)
	  ...
	  do n=1,s_natoms(sp1)
	    do m=1,3
	      avggeom(sp1,n,m)=avggeom(sp1,n,m) / s_nmols(sp1)
	    end do
	  end do
