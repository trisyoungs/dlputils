...
!	Last changes 22 April 2010 - Added sanity check, calculating total scattering function without partitioning
	...
	logical :: npt = .FALSE., calcstdev = .FALSE., broaden = .FALSE.
	integer, allocatable :: frameadded(:), kvectors(:,:), slaveadded(:), totaladded(:)
	real*8 :: kcut,magx,magy,magz,mag, numdensity, sd, sumxsq, avg, broadenfactor = 0.0
	...
	if (nargs.LT.2) then
	  ...
	  write(0,*) "            [-stdev maxframes] [-npt] [-broaden <factor> <n>]"
	  stop
	end if
	...
	if (nargs.GE.3) then
	  ...
	  do
	    ...
	    select case (temp)
	      ...
	      case ("-broaden");
		broaden = .TRUE.
		n = n + 1; call getarg(n,temp); read(temp,"(f12.4)") broadenfactor
		n = n + 1; call getarg(n,temp); read(temp,"(i6)") nbroaden
	      case ("-p4amslave")
