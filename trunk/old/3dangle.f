...
!	and the defining X (phi) and Y (theta) axes of the molecule.
	...
	real*8 angle(0:GRID,0:GRID),hist(2,0:GRID),a,b,c
...
101	success=readframe(10,fileform)
	...
	DO m=1,s_nmols(1)
	  ...
	  DO n=1,s_nmols(angspecies)
	    ! Rotate the molecule into the local coordinate system first...
	    CALL PBC(ox(angspecies,n),oy(angspecies,n),oz(angspecies,n),
     +	ox(1,m),oy(1,m),oz(1,m),tx,ty,tz,cell)
	    tx=tx-ox(1,m)
	    ty=ty-oy(1,m)
	    tz=tz-oz(1,m)
	    ...
	    ! Now rotate the atoms of the molecule the same way.
	    ! Must take into account the changed position of the COM origin...
	    DO k=1,s_natoms(angspecies)
	      CALL PBC(xpos(o+k-1),ypos(o+k-1),zpos(o+k-1),ox(angspecies,n),
     +	  oy(angspecies,n),oz(angspecies,n),tx,ty,tz,cell)   ! PBC to local COM
	      tx=tx-ox(angspecies,n)
	      ty=ty-oy(angspecies,n)
	      tz=tz-oz(angspecies,n)
	    ...
	    END DO
	    ! Now both the COm and the atomic positions relative to the COM have been transformed.
	    ! Store the position that we're interested in... (use px,py,pz)
	    ...
	    py=oy(angspecies,n)
	    pz=oz(angspecies,n)
	    tx=xpos(p+7)     ! Should be unique hydrogen atom...
	    ty=ypos(p+7)
	    tz=zpos(p+7)
	    ...
	    a=SQRT((ox(1,m)-tx)**2 + (oy(1,m)-ty)**2 + (oz(1,m)-tz)**2)
	    b=SQRT((tx-px)**2 + (ty-py)**2 + (tz-pz)**2)
	    c=SQRT((ox(1,m)-px)**2 + (oy(1,m)-py)**2 + (oz(1,m)-pz)**2)
	    phi=(a**2 + b**2 - c**2) / (2*a*b)
	    phi=radcon*ACOS(phi)
	    n1=NINT(phi)
