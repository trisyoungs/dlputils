...
!	Takes CLI argument to skip frames at start
	...
	integer :: framestodo, count1, sp, selector, framestoskip, framesskipped
	integer :: iargc, igeom(6), newflag
	integer, allocatable :: aflag(:,:)
	logical :: reverselogic = .false.
	real*8, allocatable :: msd(:,:), point(:), finalmsd(:,:)
	real*8, allocatable :: x(:,:), y(:,:), z(:,:)
	integer, allocatable :: orn(:)
	real*8 :: tx,ty,tz,rij2,deltat, geom(6), vec(3), vec1(3), vec2(3), mag, xx, yy, zz, dist, dx, dy, dz

	nargs = iargc()
	if (nargs.lt.7) then
	  write(0,"(A)") "Usage : msd <DLP HISTORYfile> <DLP OUTPUTfile> <delta t> <framestodo> <framestoskip> <species> <[-]selector> [extra data]"
	  stop
	end if
	...
	call getarg(6,temp); read(temp,"(I6)") sp
	call getarg(7,temp); read(temp,"(I6)") selector
	...
	do i=8,nargs
	  ...
	  if (n.lt.20) read(temp,"(f10.6)") geom(i-7)
	  if (n.gt.20) read(temp,"(i10)") igeom(i-7)
	end do
...
50     if (baselen.EQ.-1) then
	...
	select case (selector)
	  ...
	  case (5); write(9,"(a,f9.4,a,i2)") "# ZProximity - All molecules within ",geom(1)," of species ",igeom(2)
		    ...
		    geom(1) = geom(1) * geom(1)
	  case (6); write(9,"(a,'(',i2,'/',i2,'->',i2,')')") "# ZOrient - Positive vectors of atoms ",igeom(1),igeom(2),igeom(3)
	  	    write(9,"(a,f9.4,a,f9.4)") "#              between z = ",geom(4)," and ",geom(5)
		    vec(1) = igeom(1)
		    vec(2) = igeom(2)
		    vec(3) = igeom(3)
	  case (7); write(9,"(a,'(',i2,'/',i2,'->',i2,')')") "# ZOrient - Negative vectors of atoms ",igeom(1),igeom(2),igeom(3)
	  	    write(9,"(a,f9.4,a,f9.4)") "#              between z = ",geom(4)," and ",geom(5)
		    vec(1) = igeom(1)
		    vec(2) = igeom(2)
		    vec(3) = igeom(3)
...
101	success=readframe()
	...
	if (framesskipped.le.framestoskip) goto 101
	nframes=nframes+1
...
801	write(0,*) ""
	...
	do i=1,nframes
	  do m1=1,s_nmols(sp)
	    if (aflag(i,m1).eq.2) aflag(i,m1) = 1
	  end do
	end do
