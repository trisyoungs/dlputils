!	** raw2sq **
!	Compute F(Q) from raw, unweighted partials
...
2       if (baselen.EQ.-1) THEN
	...
	if (readmap) then
	  ...
	  do n=1,natms
	    read(15,"(A6)",err=50,end=50) atmname(n)
	  end do
	  goto 55
...
55	  close(15)
	...
	allocate(sq(nbins), totaladded(nbins))
	...
	do alpha=1,ntypes
	  do beta=1,ntypes
	    ...
	    ! No need to divide by bin population and natms since this is already done by the 'sq' program
		...
		write(9,"(a,1x,F8.5,3x,E14.5,2x,I8)") c,(n*binwidth)-binwidth*0.5, partialsq(alpha,beta,n), totaladded(n)
	      end do
	      close(9)
	    end if
	  end do
	...
	OPEN(UNIT=9,file=resfile,FORM="FORMATTED")
	write(9,"(a,i8)") "# F(Q) data follows (ds/dS - selfscatter)."
